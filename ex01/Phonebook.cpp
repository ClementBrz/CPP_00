...
/*   Updated: 2023/10/24 14:27:12 by cbernaze         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		if (contact_tmp.last_name() == SUCCESS)
			if (contact_tmp.nickname() == SUCCESS)
				if (contact_tmp.phone_number() == SUCCESS)
					...
					{
						this->_contact[index] = contact_tmp;
...
int	Phonebook::search_contact()
{
	...
	{
		std::cout << "|---|----------|----------|----------|" << std::endl;
		this->_contact[i].show_contacts(i + 1);
	}
	...
	while (index < 1 || index > 8)
	{
		...
		if (!input.empty() && std::istringstream(input) >> index && index > 0 && index < 8)
			this->_contact[index - 1].display_contact();
		else
...
{
	this->_contact[0] = Contact("Clement", "Bernazeau", "Kiki", "Aime les vaches", "0553");
	this->_contact[1] = Contact("Pablo", "Cheron", "Pablito", "Joue trop a pokemon", "0554");
	this->_contact[2] = Contact("Lisa Estelle", "Duheron", "David Finshark", "Aime les dinosaures", "0555");
	this->_contact[3] = Contact("Alban", "Tardif", "Albibou", "A trop regarde le Parrain", "0556");
	this->_contact[4] = Contact("Axel", "Kusniak", "Axel Vierge", "N'est jamais la", "0557");
	this->_contact[5] = Contact("Alan", "De Freitas", "Phoenix des hotes de ses bois", "Bouffe trop au resto", "0558");
	this->_contact[6] = Contact("Alban", "Montigny", "Alban Minishell", "Ne sait pas coder", "0559");
}